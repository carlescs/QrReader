...
class UpdateBarcodeUseCaseTest {
    @Test
    fun `calls repository update and returns rows`() = runTest {
        val barcode = BarcodeModel(id = 1, date = Date(), type = 1, format = 1, barcode = "abc")
        var lastUpdated: BarcodeModel? = null
        ...
        val rows = uc(barcode)
        assertEquals(3, rows)
        assertEquals(barcode, lastUpdated)
    }
}
