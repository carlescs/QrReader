...
/**
 * Displays text shared to the app as if it had been scanned from a barcode.
 * URLs are rendered as tappable links. Any content can be saved to history.
 */
@Composable
fun SharedTextContent(
    rawText: String,
    ...
    Title(title = if (isUrl) stringResource(R.string.url) else stringResource(R.string.text))
    ...
    if (isUrl) {
        Text(
            ...
            modifier = Modifier.clickable { uriHandler.openUri(rawText) }
        )
    } else {
        Text(text = rawText)
    }
                ...
                coroutineScope.launch {
                    try {
                        ...
                        if (duplicateCheckEnabled) {
                            val existing = checkDuplicateBarcodeUseCase(rawText)
                            if (existing != null) {
                        ...
                        val barcodeModel = BarcodeModel(
                            date = Date(),
                            type = if (isUrl) Barcode.TYPE_URL else Barcode.TYPE_TEXT,
                            barcode = rawText,
